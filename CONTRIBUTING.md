...
Этот проект использует локальную базу данных **PostgreSQL**, к которой приложение
подключается напрямую через SQLAlchemy. Ниже представлена актуальная структура
таблиц и их полей, чтобы вы понимали логику хранения данных.
...
- `id` (uuid, PK, default uuid_generate_v4 на стороне приложения)
- `name` (text, not null)
- `code` (text, unique, not null) — короткий код для подключения
- `captain_id` (int, FK → users.id)
...
## 📌 Примечания
- UUID создаются в приложении (Python) и сохраняются в базу как обычные значения.
...
## 📌 Работа с базой
- В проекте используется SQLAlchemy для доступа к PostgreSQL.
- Конфигурация подключения задаётся через переменную окружения `DATABASE_URL`.
- Инициализировать структуру таблиц можно командой `python -m scripts.init_db`.
